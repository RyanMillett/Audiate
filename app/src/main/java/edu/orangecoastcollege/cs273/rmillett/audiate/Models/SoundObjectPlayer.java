...
public class SoundObjectPlayer {
     ...
     */
    public static final int DEFAULT_PLAYBACK_SAMPLE_RATE = 8000; // TODO: link this to general settings
    ...
    private void createWaveform(SoundObject soundObject) {
        ...
        // Create waveform
        int numSamples = (soundObject.getDurationMilliseconds() / 1000) * DEFAULT_PLAYBACK_SAMPLE_RATE;
        double[] sample = new double[numSamples];
        for (int i = 0; i < numSamples; ++i) {
            double valueSum = 0;

            for (int j = 0; j < frequencies.length; ++j) {
                valueSum += Math.sin(2 * Math.PI * i / (DEFAULT_PLAYBACK_SAMPLE_RATE / frequencies[j]));
            }
    ...
    private void createSequencedWaveform(ChordScale chordScale) {
        ...
        for (final Note note : sequence) {
            int num = note.getDurationMilliseconds() * DEFAULT_PLAYBACK_SAMPLE_RATE / 1000;
            double[] sample = new double[num];

            for (int i = 0; i < num; ++i) {
                sample[i] = Math.sin(2 * Math.PI * i * note.getPitchFrequency() / DEFAULT_PLAYBACK_SAMPLE_RATE);
            }
            samples.add(sample);
            numSamples += num;
        }
    ...
    private void writeAudioTrack(byte[] generatedSnd, int numSamples) {
        mAudioTrack = new AudioTrack(AudioManager
                .STREAM_MUSIC, DEFAULT_PLAYBACK_SAMPLE_RATE,
